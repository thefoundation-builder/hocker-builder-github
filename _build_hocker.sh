    ...
    ## APT CACHE DOCKER
        ...
        if [ "$foundcache" = "yes" ]  ;then
          echo "FOUND CACHE CONTAINER, GETTING IP:PORT"
            proxyaddr=$(
                (
                docker inspect ultra-apt-cacher |jq .[].NetworkSettings.Networks -c|jq .[].IPAddress --raw-output|grep -v ^$|sort -u |while read testip;do echo testing apt-cache IP $testip:80    >&2 ;  curl --timeout 5 -s $testip:80/|grep -i apt|grep -i -q cache && echo $testip:80 ;done|head -n1
                docker inspect apt-cacher-ng    |jq .[].NetworkSettings.Networks -c|jq .[].IPAddress --raw-output|grep -v ^$|sort -u |while read testip;do echo testing apt-cache IP $testip:3142  >&2 ;  curl --timeout 5 -s $testip:3142/|grep -qi "apt-cacher" && echo $testip:3142 ;done|head -n1
                ) |head -n1
            )
